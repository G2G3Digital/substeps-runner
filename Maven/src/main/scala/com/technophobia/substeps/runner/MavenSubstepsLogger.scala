...
import com.technophobia.substeps.domain.BasicScenario
...
class MavenSubstepsLogger(logger: Log, val expectedNumberOfFeatured: Int) extends DomainEventSubscriber {
  ...
  def handle(event: SubstepsDomainEvent) {
    ...
    event match {
      ...
      case ExecutionStarted(Feature(name, _, _, _), time) =>
      ...
      case ExecutionCompleted(Feature(name, _, _, _), runResult, time) =>
